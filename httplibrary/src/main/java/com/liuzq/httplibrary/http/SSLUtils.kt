...
object SSLUtils {
    ...
    private fun getSslSocketFactoryBase(trustManager: X509TrustManager?, bksFile: InputStream?, password: String?, vararg certificates: InputStream): SSLParams {
        ...
        try {
            val keyManagers: Array<out KeyManager>? = prepareKeyManager(bksFile, password)
            val trustManagers: Array<TrustManager>? = prepareTrustManager(*certificates)
            var manager: X509TrustManager? = null
            manager = when {
                manager != null -> trustManager //优先使用用户自定义的TrustManager
                trustManagers != null -> chooseTrustManager(trustManagers) //然后使用默认的TrustManager
                else -> UnSafeTrustManager //否则使用不安全的TrustManager
            }
    ...
    private fun prepareKeyManager(bksFile: InputStream?, password: String?): Array<out KeyManager>? {
        try {
            if (bksFile == null || password.isNullOrBlank()) return null
     ...
     * 这是一种有很大安全漏洞的办法
     */
    public val UnSafeTrustManager: X509TrustManager = object : X509TrustManager {
     ...
     */
    public val UnSafeHostnameVerifier: HostnameVerifier = HostnameVerifier { _, _ -> true }
}